...
/** Validates the experimental 3x3 Heitan board and shared mechanics. */
public final class Heitan3x3Validation
{
    private static final int SUPPLY_COUNT = 16;
    private static final int OBJECTIVE_COUNT = 9;
    private static final int SITE_COUNT = SUPPLY_COUNT + OBJECTIVE_COUNT;
    private static final int TOTAL_PLACEMENTS = 54;
    private static final int TURNS = 18;
    private static final int PIECES_PER_PLAYER = 27;
    private static final int ADVANTAGE_WEIGHT = 28;
    private static final int SECURED_WEIGHT = 280;

    private Heitan3x3Validation()
    {
        // Utility class.
    }
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        {
            System.err.println(
                "Usage: Heitan3x3Validation <Heitan.lud> [random-games]"
            );
            System.exit(2);
        }
        ...
        final Game game = load3x3(args[0]);
        ...
        System.out.printf(
            "3x3 validation passed: 25 sites, 36 exact Objective edges, "
            + "24 exact Supply-grid lines, shared-mechanics scenario, "
            + "scoring weights, and %d random games.%n",
            randomGames
        );
    }

    private static Game load3x3(final String path) throws Exception
    {
        final File file = new File(path).getCanonicalFile();
        final Game game = GameLoader.loadGameFromFile(
            file, List.of("Board/3x3")
        );
        if (game == null)
    ...
    private static void validateStructure(final Game game)
    {
        if (game.board().graph().vertices().size() != SITE_COUNT)
            fail("Expected 25 graph vertices.");
        if (game.board().graph().edges().size() != OBJECTIVE_COUNT * 4)
            fail("Expected 36 Objective-to-Supply edges.");
    ...
    )
    {
        ...
        if (lineItems.size() != 24 || !actualLines.equals(expectedLines))
        {
            fail(
                "Supply grid graphics differ: expected 24 exact adjacent lines, got "
                + lineItems.size() + "."
            );
        }
    }
    ...
    private static void validateDeterministicMechanics(final Game game)
    {
        ...
        applySupply(game, context, supply(3, 3));
        assertMover(context, 2);
        assertState(context, supply(0, 0), 1);

        playFillerTurn(game, context, supply(3, 0), supply(0, 3));
        ...
        playFillerTurn(game, context, supply(2, 0), supply(3, 2));
        ...
        playFillerTurn(game, context, supply(1, 0), supply(2, 3));
        ...
        playFillerTurn(game, context, supply(2, 1), supply(3, 2));
    ...
    private static void validateRandomGames(final Game game, final int gameCount)
    {
        ...
        {
            final long seed = 610000L + gameIndex;
    ...
    {
        return SUPPLY_COUNT + row * 3 + column;
    }
