...
def build_diagnostics(config: dict,rows: list[dict[str,str]],paths: dict[tuple[int,int],Path]) -> dict:
    ...
    return {"game_diagnostics":len(games),"late_reversal_rows":len(reversals),"central_comparison_rows":len(central_rows),"structural_summary":structural,"late_reversal_summary":summary,"central_supply_summary":central_summary}
...
def render_report(analysis: dict) -> str:
    ...
    for row in analysis["difference_in_differences"]:
    ...
    lines += ["","## Preregistered interpretation","",f"- Primary 100k drop: corrected 100k minus corrected 30k.",f"- Auxiliary drop label: **{analysis['classification']['drop_classification']}**.",f"- Search-depth classification: **{analysis['classification']['depth_classification']}**.","- The primary drop remains negative by point estimate, but its interval reaches zero; the drop label is therefore unresolved.","- No material-change label is used.","","## Structural diagnostics","","| UCT | Secured / Advantage / Piece / Draw decisions | Mean margins S / A / Piece | Persistent-lead games | Mean first persistent turn |","|---:|---:|---:|---:|---:|"]
    ...
    lines += ["","### Early central Supply","","| Sample | UCT | Early placement frequency | Mean early placements | Mean first commitment turn |","|---|---:|---:|---:|---:|"]
    for row in analysis["secondary"]["central_supply_summary"]:lines.append(f"| {row['sample']} | {row['iteration_limit']:,} | {pct(row['early_central_placement_frequency'])}% | {float(row['mean_early_central_placement_count']):.2f} | {float(row['mean_first_commitment_turn_conditional']):.2f} |")
    lines += ["","Under the preregistered four-point central group, every P1 committed on its first turn in every tested sample. The mean early-placement count does not reproduce a reduced 100k central-Supply priority; this diagnostic is saturated and should not be over-interpreted.","","## Incomplete games","",("No production tasks are incomplete." if not analysis["failures"] else f"{len(analysis['failures'])} task(s) are incomplete; see `results/final/failures.csv`. Missingness may be non-random."),"","These finite self-play samples do not establish convergence, optimal play, or solved-game balance."]
    return "\n".join(lines)+"\n"
