#!/usr/bin/env python3
"""Run resilient corrected-rule Issue #108 pilot and production tasks."""
...
RUNNER = Path(__file__).with_name("Heitan3x3CorrectedExperiment.java")
REPLAYER = Path(__file__).with_name("Heitan3x3CorrectedReplay.java")
...
def expected_sites() -> set[str]:
    return {f"S{row}{column}" for row in range(4) for column in range(4)} | {f"O{row}{column}" for row in range(3) for column in range(3)}


def audit_board(value: str) -> dict[str, int | str]:
    encoded = value.split("|")
    if len(encoded) != 25:
        raise ValueError("final board must contain 25 sites")
    ...
    if set(sites) != expected_sites():
        raise ValueError("final-board site set differs from Board/3x3")
    ...
    scores = [280 * secured[player] + 28 * advantage[player] + corrected[player] for player in (0, 1)]
...
def timed_java(command: list[str], timeout: int, on_start=None) -> tuple[subprocess.CompletedProcess[str], int | None]:
    timed_command=["/usr/bin/time","-l",*command] if Path("/usr/bin/time").is_file() else command
...
def replay(trial: Path, output: Path, jar: Path, game: Path, task: protocol.Task) -> dict[str, str]:
    output.mkdir(parents=True, exist_ok=True)
    command = ["java", "-cp", str(jar), str(REPLAYER), str(game), str(trial), str(task.game_index), str(output / "games.csv"), str(output / "placements.csv"), str(output / "turn-states.csv")]
...
def validate_generated(task: protocol.Task, task_dir: Path, jar: Path, game: Path, config: dict) -> dict:
    ...
    normalize_trial(raw, normalized, config["trial_normalization"]["game_path"])
    raw_tables = task_dir / "validation-raw" / "raw-trial"
    normalized_tables = task_dir / "validation-raw" / "normalized-trial"
    raw_game = replay(raw, raw_tables, jar, game, task)
    normalized_game = replay(normalized, normalized_tables, jar, game, task)
    ...
    if result["completed"].lower() != "true" or result["end_type"] != "NaturalEnd" or (int(result["moves"]), int(result["turns"])) != (54, 18):
        raise ValueError("game is not a natural 54-placement / 18-turn completion")
    ...
    if {player: sum(int(row["mover"]) == player for row in placements) for player in (1, 2)} != {1: 27, 2: 27}:
        raise ValueError("per-player placement totals differ from 27/27")
    metrics = audit_board(result["final_board"])
    ...
    return {"schema_version": 1, "validated": True, "legal_move_replay": True, "natural_end": True, "moves": 54, "turns": 18, "all_turns_have_three_placements": True, "p1_placements": 27, "p2_placements": 27, "raw_trial_sha256": protocol.sha256(raw), "normalized_trial_sha256": protocol.sha256(normalized), "raw_to_normalized": True, "score_reconstructed": True, "winner_reconstructed": True, "metrics": metrics}
...
def run_task(task: protocol.Task, all_tasks: list[protocol.Task], config_hash: str, jar: Path, game: Path, config: dict, stop: threading.Event) -> str:
    ...
    with protocol.locked_manifest(task.namespace, all_tasks, config_hash) as manifest:
        ...
        row.update(state="running", attempts=int(row["attempts"])+1, error=None, run_owner={"pid": os.getpid(), "runner_id": protocol.opaque_runner_id(), "command_marker": "issue-108/scripts/run_experiments.py", "started_at_utc": protocol.utc_now()}, updated_at_utc=protocol.utc_now())
    ...
    try:
        ...
        def started_java(pid: int) -> None:
            protocol.update_task(task.namespace,all_tasks,config_hash,task.task_id,"running",run_owner={"pid":os.getpid(),"runner_id":protocol.opaque_runner_id(),"command_marker":"issue-108/scripts/run_experiments.py","java_pid":pid,"java_command_marker":"Heitan3x3CorrectedExperiment.java"})
...
def require_persistent_execution() -> None:
    ...
    if os.environ.get("HEITAN108_CAFFEINATE")!="1":raise ValueError("production must run under caffeinate -i with HEITAN108_CAFFEINATE=1")
    if not os.environ.get("HEITAN108_RUNNER_ID"):raise ValueError("production requires an opaque HEITAN108_RUNNER_ID")
...
def main() -> None:
    ...
    if args.namespace=="production":
        ...
        require_persistent_execution();protocol.require_production_gate(config,args.budget)
