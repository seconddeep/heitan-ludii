...
/** Validates the experimental 8x8 Heitan board and shared mechanics. */
public final class Heitan8x8Validation
{
    private static final int SUPPLY_DIM = 9;
    private static final int OBJECTIVE_DIM = 8;
    private static final int SUPPLY_COUNT = 81;
    private static final int OBJECTIVE_COUNT = 64;
    private static final int SITE_COUNT = SUPPLY_COUNT + OBJECTIVE_COUNT;
    private static final int TOTAL_PLACEMENTS = 168;
    private static final int TURNS = 56;
    private static final int PIECES_PER_PLAYER = 84;
    private static final int ADVANTAGE_WEIGHT = 85;
    private static final int SECURED_WEIGHT = 5525;

    private Heitan8x8Validation()
    {
        // Utility class.
    }
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        {
            System.err.println(
                "Usage: Heitan8x8Validation <Heitan.lud> [random-games]"
            );
            System.exit(2);
        }
        ...
        final Game game = load8x8(args[0]);
        ...
        System.out.printf(
            "8x8 validation passed: 145 sites, 256 exact Objective edges, "
            + "144 exact Supply-grid lines, shared-mechanics scenario, "
            + "scoring weights, and %d random games.%n",
            randomGames
        );
    }

    private static Game load8x8(final String path) throws Exception
    {
        final File file = new File(path).getCanonicalFile();
        final Game game = GameLoader.loadGameFromFile(
            file, List.of("Board/8x8")
        );
        if (game == null)
    ...
    private static void validateStructure(final Game game)
    {
        if (game.board().graph().vertices().size() != SITE_COUNT)
            fail("Expected 145 graph vertices.");
        if (game.board().graph().edges().size() != OBJECTIVE_COUNT * 4)
            fail("Expected 256 Objective-to-Supply edges.");
    ...
    )
    {
        ...
        if (lineItems.size() != 144 || !actualLines.equals(expectedLines))
        {
            fail(
                "Supply grid graphics differ: expected 144 exact adjacent lines, got "
                + lineItems.size() + "."
            );
        }
    }
    ...
    private static void validateDeterministicMechanics(final Game game)
    {
        ...
        applySupply(game, context, supply(8, 8));
        assertMover(context, 2);
        assertState(context, supply(0, 0), 1);

        playFillerTurn(game, context, supply(8, 0), supply(0, 8));
        ...
        playFillerTurn(game, context, supply(5, 0), supply(0, 5));
        ...
        playFillerTurn(game, context, supply(4, 0), supply(0, 4));
        ...
        playFillerTurn(game, context, supply(3, 0), supply(5, 4));
    ...
    private static void validateRandomGames(final Game game, final int gameCount)
    {
        ...
        {
            final long seed = 750000L + gameIndex;
