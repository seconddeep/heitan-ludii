#!/usr/bin/env python3
"""Frozen identities, integrity gates, and resilient manifests for Issue #112."""
...
FULL_SCORING_VALIDATION_PATH = RESULTS_ROOT / "production" / "full-scoring-validation.json"
...
def validate_config(config: dict) -> None:
    ...
    if policy["manual_override_allowed"] is not False or len(policy["rules_in_precedence_order"]) != 6:
        raise ValueError("classification precedence/manual-override policy is incomplete")
    board = config["board"]
    expected = {"supply_points": 25, "objectives": 16, "sites": 41, "pieces_per_player": 36, "placements": 72, "heitan_turns": 24, "advantage_weight": 37, "secured_weight": 629}
    if any(int(board[key]) != value for key, value in expected.items()) or config["board_option"] != "Board/4x4":
        raise ValueError("4x4 board constants differ from the preregistration")
    if config["supply_diagnostics"]["observation_unit"] != "Heitan-turn end only":
        raise ValueError("Supply diagnostic timing differs from the preregistration")
...
def opaque_runner_id() -> str:
    value = os.environ.get("HEITAN112_RUNNER_ID", "local-runner")
    if not value or any(character not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_" for character in value):
        raise ValueError("HEITAN112_RUNNER_ID must be an opaque alphanumeric, dash, or underscore identifier")
    return value
...
def validation_gate_path(budget: int) -> Path:
    return RESULTS_ROOT / "production" / "validation-gates" / f"uct-{budget:06d}.json"


def budget_rows_sha256(manifest: dict, budget: int) -> str:
    rows = {key: value for key, value in manifest["tasks"].items() if int(value["iteration_limit"]) == budget}
...
def worktree_gate(lock: dict) -> None:
    ...
    for line in status:
        ...
        if " -> " in path:
            path = path.split(" -> ", 1)[1]
        if not path.startswith("experiments/issue-112/results/"):
...
def require_production_gate(config: dict, budget: int) -> dict:
    ...
    expected_game = lock.get("game_definition_gate") or {}
    if expected_game.get("board_option") != "Board/4x4" or int(expected_game.get("advantage_weight", -1)) != 37 or int(expected_game.get("secured_weight", -1)) != 629:
        raise ValueError("source-locked Heitan.lud 4x4 scoring-weight gate is absent or differs")
    if int(config["board"]["advantage_weight"]) != int(expected_game["advantage_weight"]) or int(config["board"]["secured_weight"]) != int(expected_game["secured_weight"]):
        raise ValueError("runner configuration differs from source-locked 4x4 scoring weights")
    ...
    if manifest.exists():
        ...
        for previous in earlier:
            ...
            gate_path = validation_gate_path(previous)
            if not gate_path.is_file():
                raise ValueError(f"earlier budget {previous} lacks its validation-only gate")
            gate = load_json(gate_path)
            if gate.get("validation_scope") != "operational-only-outcome-blind" or gate.get("aggregate_outcomes_inspected") is not False:
                raise ValueError(f"earlier budget {previous} gate is not outcome-blind")
            if gate.get("budget_rows_sha256") != budget_rows_sha256(data, previous):
                raise ValueError(f"earlier budget {previous} changed after its validation-only gate")
    return lock
