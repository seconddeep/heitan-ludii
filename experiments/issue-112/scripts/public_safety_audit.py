#!/usr/bin/env python3
"""Audit Issue #112 publishable artifacts and branch commit metadata."""
...
def publishable_files() -> list[Path]:
    completed=subprocess.run(["git","ls-files","--cached","--others","--exclude-standard","experiments/issue-112","experiments/issue-112.md"],cwd=protocol.REPO_ROOT,text=True,capture_output=True,check=True)
    return [protocol.REPO_ROOT/line for line in completed.stdout.splitlines() if line and (protocol.REPO_ROOT/line).is_file()]
...
def main() -> None:
    ...
    output=protocol.RESULTS_ROOT/"final"/"public-safety-audit.json";protocol.atomic_write_json(output,report)
    artifact=protocol.RESULTS_ROOT/"final"/"artifact-manifest.json";environment=protocol.RESULTS_ROOT/"final"/"environment.json"
    if artifact.is_file():
        value=protocol.load_json(artifact);value.setdefault("outputs",{})[output.relative_to(protocol.REPO_ROOT).as_posix()]=protocol.sha256(output);protocol.atomic_write_json(artifact,value)
    if environment.is_file():
        value=protocol.load_json(environment);value["artifact_manifest_sha256"]=protocol.sha256(artifact);value["output_hashes"]={path.relative_to(protocol.REPO_ROOT).as_posix():protocol.sha256(path) for path in sorted(output.parent.iterdir()) if path.is_file() and path.name not in {"environment.json","artifact-manifest.json"}};protocol.atomic_write_json(environment,value)
