...
/** Deterministic corrected-rule Board/4x4 self-play runner for Issue #112. */
public final class Heitan4x4CorrectedExperiment
{
    private static final int SUPPLIES=25, OBJECTIVES=16, SITES=41, PIECES=36, MOVES=72, TURNS=24;

    public static void main(final String[] args) throws Exception
    {
        if(args.length!=9){System.err.println("Usage: Heitan4x4CorrectedExperiment <game> <id> <agent> <seed> <iterations> <result.csv> <trial> <index> <raw-game-path>");System.exit(2);}
        ...
        final Path result=Path.of(args[5]),trialPath=Path.of(args[6]);final List<String> options=List.of("Board/4x4");
        final Game game=GameLoader.loadGameFromFile(gameFile,options);
        if(game==null||game.board().graph().vertices().size()!=SITES)throw new IllegalStateException("Board/4x4 did not load as 41 sites");
    ...
    private static void validate(Context c,Trial t,Metrics m){if(!t.over()||!"NaturalEnd".equals(t.status().endType().toString())||t.numMoves()!=MOVES||t.numTurns()!=TURNS)throw new IllegalStateException("Unexpected game end");if(m.p1Total!=PIECES||m.p2Total!=PIECES)throw new IllegalStateException("Unexpected piece totals");}
