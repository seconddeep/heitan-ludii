...
/** Legal replay and turn-end state extraction for one Issue #112 trial. */
public final class Heitan4x4CorrectedReplay
{
    private static final int SUPPLIES=25,SITES=41,MOVES=72,TURNS=24;
    public static void main(String[] args)throws Exception{
        if(args.length!=7){System.err.println("Usage: Heitan4x4CorrectedReplay <game> <trial> <game-index> <games.csv> <placements.csv> <turn-states.csv> <operational|full>");System.exit(2);}
        File gameFile=new File(args[0]).getCanonicalFile();Path trialPath=Path.of(args[1]);int gameIndex=Integer.parseInt(args[2]);boolean full="full".equals(args[6]);if(!full&&!"operational".equals(args[6]))throw new IllegalArgumentException("Unknown validation mode");Game game=GameLoader.loadGameFromFile(gameFile,List.of("Board/4x4"));if(game==null||game.board().graph().vertices().size()!=SITES)throw new IllegalStateException("Wrong board");
        try(BufferedWriter games=writer(args[3]);BufferedWriter placements=writer(args[4]);BufferedWriter states=writer(args[5])){row(games,List.of("game_index","winner","moves","turns","end_type","final_p1_score","final_p2_score","final_board"));row(placements,List.of("game_index","turn_number","placement_number","mover","target","target_type","supply_source"));row(states,List.of("game_index","turn_number","mover","point","point_type","state_at_turn_end","p1_at_turn_end","p2_at_turn_end"));replay(game,trialPath,gameIndex,games,placements,states,full);}
    }
    private static void replay(Game game,Path path,int gameIndex,BufferedWriter games,BufferedWriter placements,BufferedWriter states,boolean full)throws Exception{Trial source=MatchRecord.loadMatchRecordFromTextFile(path.toFile(),game).trial();if(source.numMoves()!=MOVES||source.numTurns()!=TURNS||!source.over()||!"NaturalEnd".equals(source.status().endType().toString()))throw new IllegalStateException("Incomplete source");Trial replay=new Trial(game);Context context=new Context(game,replay);game.start(context);int sourceMove=0;for(int turn=1;turn<=TURNS;turn++){int mover=context.state().mover();int before=total(context);for(int placement=1;placement<=3;placement++){if(context.state().mover()!=mover)throw new IllegalStateException("Mover changed before third placement");Move recorded=source.getMove(sourceMove++),legal=legal(game,context,recorded);if(legal==null)throw new IllegalStateException("Illegal recorded move "+sourceMove);int[] decision=decision(legal);row(placements,List.of(Integer.toString(gameIndex),Integer.toString(turn),Integer.toString(placement),Integer.toString(mover),name(decision[0]),decision[0]<SUPPLIES?"supply":"objective",decision[1]<0?"":name(decision[1])));game.apply(context,legal);}if(total(context)!=before+3)throw new IllegalStateException("Turn did not add exactly three Pieces");Snapshot snapshot=snapshot(context);for(int site=0;site<SITES;site++)row(states,List.of(Integer.toString(gameIndex),Integer.toString(turn),Integer.toString(mover),name(site),site<SUPPLIES?"supply":"objective",Integer.toString(snapshot.state[site]),Integer.toString(snapshot.p1[site]),Integer.toString(snapshot.p2[site])));}if(sourceMove!=MOVES||!replay.over())throw new IllegalStateException("Replay did not complete naturally");Snapshot end=snapshot(context);int[] score=score(end);if(full&&(score[0]!=context.score(1)||score[1]!=context.score(2)||replay.status().winner()!=source.status().winner()))throw new IllegalStateException("Full score or winner reconstruction differs");row(games,List.of(Integer.toString(gameIndex),Integer.toString(replay.status().winner()),Integer.toString(replay.numMoves()),Integer.toString(replay.numTurns()),replay.status().endType().toString(),Integer.toString(score[0]),Integer.toString(score[1]),end.board));}
    ...
    private static int[] score(Snapshot s){int[] result=new int[2];for(int site=SUPPLIES;site<SITES;site++){if(s.state[site]==3)result[0]+=629;else if(s.state[site]==4)result[1]+=629;else if(s.state[site]==1){result[0]+=37+s.p1[site];}else if(s.state[site]==2){result[1]+=37+s.p2[site];}}return result;}
    private static String name(int site){return site<SUPPLIES?"S"+(site/5)+(site%5):"O"+((site-SUPPLIES)/4)+((site-SUPPLIES)%4);}
