#!/usr/bin/env python3
"""Protocol, scoring, normalization, and resume tests for Issue #112."""
...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_primary_depth_contrast_and_no_material_label(self):
        analysis=protocol.load_config()["analysis"]
        self.assertEqual(analysis["primary_depth_contrast"],"corrected 100k - corrected 30k");self.assertFalse(analysis["material_change_label_enabled"]);self.assertFalse(analysis["historical_pattern_label_policy"]["manual_override_allowed"])

    def test_turn_end_supply_definitions(self):
        policy=protocol.load_config()["supply_diagnostics"]
        self.assertEqual(policy["observation_unit"],"Heitan-turn end only");self.assertIn("72",policy["placement_share_definition"])

    def test_source_game_exposes_expected_4x4_weights(self):
        gate=freeze_protocol.game_definition_gate(protocol.load_config())
        self.assertEqual((gate["advantage_weight"],gate["secured_weight"]),(37,629));self.assertEqual(gate["status"],"passed")


class ScoringTests(unittest.TestCase):
    def board(self):
        values=[f"S{r}{c}:0:0:0" for r in range(5) for c in range(5)]+[f"O{r}{c}:0:0:0" for r in range(4) for c in range(4)]
        return values

    def test_only_own_advantage_pieces_score(self):
        values=self.board();values[25]="O00:1:2:1";values[26]="O01:2:1:2";metrics=run_experiments.audit_board("|".join(values))
    ...
    def test_secured_and_advantage_precede_pieces(self):
        values=self.board();values[25]="O00:3:3:0";values[26]="O01:2:0:3";metrics=run_experiments.audit_board("|".join(values));self.assertEqual(metrics["winner"],1)
        values=self.board();values[25]="O00:1:1:0";values[26]="O01:2:0:3";values[27]="O02:1:1:0";metrics=run_experiments.audit_board("|".join(values));self.assertEqual(metrics["winner"],1)

    def test_draw_when_all_layers_tie(self):
        self.assertEqual(run_experiments.audit_board("|".join(self.board()))["winner"],0)

    def test_weights_preserve_4x4_lexicographic_order(self):
        self.assertGreater(37,36);self.assertGreater(629,16*37+36)

    def test_own_secured_identity_is_enforced(self):
        values=self.board();values[25]="O00:3:2:0"
