...
while($pending.Count-or$running.Count){
    while($pending.Count-and$running.Count-lt$Parallelism){$task=$pending.Dequeue();$e=$task.experiment;$arguments=@('-cp',$LudiiJar,$runner,$game,$e.id,$e.piece_budget,$e.agent,$task.games,$task.seed,$e.iteration_limit,$task.raw,$task.trials,$repo,$task.offset);$process=Start-Process -FilePath java -ArgumentList $arguments -NoNewWindow -PassThru;$running+=[pscustomobject]@{process=$process;task=$task}}
...
foreach($experiment in $config.experiments){
    ...
    $spec=$config.piece_configurations."$($experiment.piece_budget)"
    if(@($rows|Where-Object{$_.completed-ne'true'-or[int]$_.piece_budget-ne[int]$experiment.piece_budget-or$_.end_type-ne'NaturalEnd'-or[int]$_.moves-ne[int]$spec.total_placements-or[int]$_.turns-ne[int]$spec.turns-or[int]$_.p1_total_pieces-ne[int]$experiment.piece_budget-or[int]$_.p2_total_pieces-ne[int]$experiment.piece_budget}).Count){throw "Structural generation validation failed: $($experiment.id)"}
