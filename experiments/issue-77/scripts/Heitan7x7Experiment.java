...
/** Deterministic headless runner for Issue 77's 7x7 resource budgets. */
public final class Heitan7x7Experiment
{
    private static final int SUPPLIES=64, OBJECTIVES=49, SITES=113;

    public static void main(final String[] args) throws Exception
    {
        if(args.length!=11){System.err.println("Usage: Heitan7x7Experiment <game> <id> <piece-budget> <agent> <games> <seed-first> <iterations> <raw.csv> <trials-dir> <repo-root> <index-offset>");System.exit(2);}
        final File gameFile=new File(args[0]).getCanonicalFile();
        final String id=args[1],agentName=args[3];final Spec spec=Spec.forBudget(Integer.parseInt(args[2]));
        final int games=Integer.parseInt(args[4]),iterations=Integer.parseInt(args[6]),offset=Integer.parseInt(args[10]);
        final long seedFirst=Long.parseLong(args[5]);
        final Path raw=Path.of(args[7]),trialsDir=Path.of(args[8]),repo=Path.of(args[9]).toAbsolutePath().normalize();
        final List<String> options=List.of(spec.option());
        final Game game=GameLoader.loadGameFromFile(gameFile,options);
        if(game==null||game.board().graph().vertices().size()!=SITES)throw new IllegalStateException("Issue 77 7x7 configuration did not load");
        ...
        try(BufferedWriter out=Files.newBufferedWriter(raw,StandardCharsets.UTF_8)){
            row(out,List.of("experiment_id","piece_budget","game_index","seed","agent","iteration_limit","completed","end_type","winner","moves","turns","p1_score","p2_score","p1_total_pieces","p2_total_pieces","p1_supply_pieces","p2_supply_pieces","p1_objective_pieces","p2_objective_pieces","p1_secured_supply","p2_secured_supply","p1_secured_objectives","p2_secured_objectives","p1_advantage_objectives","p2_advantage_objectives","deciding_criterion","elapsed_seconds","final_board","trial_file"));
            for(int index=0;index<games;index++){
                ...
                final Metrics m=metrics(context);validate(context,trial,m,spec);
                row(out,List.of(id,Integer.toString(spec.pieces),Integer.toString(number),Long.toString(seed),agentName,Integer.toString(iterations),Boolean.toString(trial.over()),trial.status().endType().toString(),Integer.toString(trial.status().winner()),Integer.toString(trial.numMoves()),Integer.toString(trial.numTurns()),Integer.toString(context.score(1)),Integer.toString(context.score(2)),Integer.toString(m.p1Total),Integer.toString(m.p2Total),Integer.toString(m.p1Supply),Integer.toString(m.p2Supply),Integer.toString(m.p1Objective),Integer.toString(m.p2Objective),Integer.toString(m.p1SecSupply),Integer.toString(m.p2SecSupply),Integer.toString(m.p1SecObj),Integer.toString(m.p2SecObj),Integer.toString(m.p1AdvObj),Integer.toString(m.p2AdvObj),criterion(m),String.format(Locale.ROOT,"%.3f",(System.nanoTime()-started)/1e9),m.board,repo.relativize(trialPath.toAbsolutePath().normalize()).toString().replace('\\','/')));
                out.flush();for(int p=1;p<=2;p++)agents.get(p).closeAI();
                System.out.printf(Locale.ROOT,"%s game %d completed in %.3fs%n",id,number,(System.nanoTime()-started)/1e9);
            }
        }
    }
    ...
    private static void validate(Context c,Trial t,Metrics m,Spec spec){if(!t.over()||!"NaturalEnd".equals(t.status().endType().toString())||t.numMoves()!=spec.moves||t.numTurns()!=spec.turns)throw new IllegalStateException("Unexpected game end");if(m.p1Total!=spec.pieces||m.p2Total!=spec.pieces)throw new IllegalStateException("Unexpected piece totals");int s1=spec.securedWeight*m.p1SecObj+spec.advantageWeight*m.p1AdvObj+m.p1Objective,s2=spec.securedWeight*m.p2SecObj+spec.advantageWeight*m.p2AdvObj+m.p2Objective;if(c.score(1)!=s1||c.score(2)!=s2)throw new IllegalStateException("Score mismatch");int winner=s1==s2?0:s1>s2?1:2;if(t.status().winner()!=winner)throw new IllegalStateException("Winner mismatch");}
    ...
    private static String name(int site){return site<SUPPLIES?"S"+(site/8)+(site%8):"O"+((site-SUPPLIES)/7)+((site-SUPPLIES)%7);}
    ...
    private record Spec(int pieces,int moves,int turns,int advantageWeight,int securedWeight){String option(){return "Board/7x7"+(pieces==72?"":"-"+pieces);}static Spec forBudget(int pieces){return switch(pieces){case 72->new Spec(72,144,48,73,3650);case 84->new Spec(84,168,56,85,4250);case 96->new Spec(96,192,64,97,4850);default->throw new IllegalArgumentException("Unsupported piece budget "+pieces);};}}
}
