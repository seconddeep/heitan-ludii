...
/** Validates the experimental 6x6 Heitan board and shared mechanics. */
public final class Heitan6x6Validation
{
    private static final int SUPPLY_COUNT = 49;
    private static final int OBJECTIVE_COUNT = 36;
    ...
    private static final int SECURED_WEIGHT = 2701;

    private Heitan6x6Validation()
    {
        // Utility class.
    }
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        {
            System.err.println(
                "Usage: Heitan6x6Validation <Heitan.lud> [random-games]"
            );
            System.exit(2);
        }
        ...
        final Game game = load6x6(args[0]);
    ...
    private static Game load6x6(final String path) throws Exception
    {
        final File file = new File(path).getCanonicalFile();
        final Game game = GameLoader.loadGameFromFile(
            file, List.of("Board/6x6")
        );
        if (game == null)
    ...
    private static void validateStructure(final Game game)
    {
        if (game.board().graph().vertices().size() != SITE_COUNT)
            fail("Expected 85 graph vertices.");
        if (game.board().graph().edges().size() != OBJECTIVE_COUNT * 4)
            fail("Expected 144 Objective-to-Supply edges.");
    ...
    private static void validateDeterministicMechanics(final Game game)
    {
        ...
        applySupply(game, context, supply(6, 6));
        assertMover(context, 2);
        assertState(context, supply(0, 0), 1);

        playFillerTurn(game, context, supply(6, 0), supply(0, 6));
    ...
    private static void validateRandomGames(final Game game, final int gameCount)
    {
        ...
        {
            final long seed = 510000L + gameIndex;
    ...
    {
        return SUPPLY_COUNT + row * 6 + column;
    }
