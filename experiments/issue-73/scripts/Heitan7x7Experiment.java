...
/** Deterministic headless runner for the validated Board/7x7 option. */
public final class Heitan7x7Experiment
{
    ...
    private static final int ADVANTAGE_WEIGHT=73, SECURED_WEIGHT=3650;

    public static void main(final String[] args) throws Exception
    {
        if(args.length!=10){System.err.println("Usage: Heitan7x7Experiment <game> <id> <agent> <games> <seed-first> <iterations> <raw.csv> <trials-dir> <repo-root> <index-offset>");System.exit(2);}
        ...
        final List<String> options=List.of("Board/7x7");
        final Game game=GameLoader.loadGameFromFile(gameFile,options);
        if(game==null||game.board().graph().vertices().size()!=SITES)throw new IllegalStateException("Board/7x7 did not load as a 113-site game");
