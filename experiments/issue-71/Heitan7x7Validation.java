...
/** Validates the experimental 7x7 Heitan board and shared mechanics. */
public final class Heitan7x7Validation
{
    private static final int SUPPLY_COUNT = 64;
    private static final int OBJECTIVE_COUNT = 49;
    private static final int SITE_COUNT = SUPPLY_COUNT + OBJECTIVE_COUNT;
    private static final int TOTAL_PLACEMENTS = 144;
    private static final int TURNS = 48;
    private static final int PIECES_PER_PLAYER = 72;
    private static final int ADVANTAGE_WEIGHT = 73;
    private static final int SECURED_WEIGHT = 3650;

    private Heitan7x7Validation()
    {
        // Utility class.
    }
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        {
            System.err.println(
                "Usage: Heitan7x7Validation <Heitan.lud> [random-games]"
            );
            System.exit(2);
        }
        ...
        final Game game = load7x7(args[0]);
        ...
        System.out.printf(
            "7x7 validation passed: 113 sites, 196 exact Objective edges, "
            + "112 exact Supply-grid lines, shared-mechanics scenario, "
            + "scoring weights, and %d random games.%n",
            randomGames
        );
    }

    private static Game load7x7(final String path) throws Exception
    {
        final File file = new File(path).getCanonicalFile();
        final Game game = GameLoader.loadGameFromFile(
            file, List.of("Board/7x7")
        );
        if (game == null)
    ...
    private static void validateStructure(final Game game)
    {
        if (game.board().graph().vertices().size() != SITE_COUNT)
            fail("Expected 113 graph vertices.");
        if (game.board().graph().edges().size() != OBJECTIVE_COUNT * 4)
            fail("Expected 196 Objective-to-Supply edges.");
    ...
    )
    {
        ...
        if (lineItems.size() != 112 || !actualLines.equals(expectedLines))
        {
            fail(
                "Supply grid graphics differ: expected 112 exact adjacent lines, got "
                + lineItems.size() + "."
            );
        }
    }
    ...
    private static void validateDeterministicMechanics(final Game game)
    {
        ...
        applySupply(game, context, supply(7, 7));
        assertMover(context, 2);
        assertState(context, supply(0, 0), 1);

        playFillerTurn(game, context, supply(7, 0), supply(0, 7));
    ...
    private static void validateRandomGames(final Game game, final int gameCount)
    {
        ...
        {
            final long seed = 710000L + gameIndex;
    ...
    {
        return SUPPLY_COUNT + row * 7 + column;
    }
